...
public sealed class CollisionDetectionSystem(World world, IParallelRunner runner)
    : AEntitySetSystem<GameState>(world.GetEntities().With((in Collidable c) => !c.Passive).AsSet(), runner)
{
    private readonly IEnumerable<Entity> _targets = world.GetEntities().With<Collidable>().AsEnumerable();
    ...
    protected override void Update(GameState state, in Entity entity)
    {
        ...
        foreach (var target in _targets)
        {
            ...
            world.Publish(new CollisionMessage(entity, target, contactPoint, contactNormal, contactTime));
        }
    }
        ...
        out Vector2 contactPoint, out Vector2 contactNormal, out float contactTime)
    {
        ...
        // return potentialCollision && contactTime < 1.0f; // for button to work
    }
}